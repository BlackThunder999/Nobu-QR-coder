(function() {
    ...
    // === ДИНАМИЧЕСКИЕ СТИЛИ ДЛЯ АДМИНКИ (чтобы кнопки были видны) ===
    const styleTag = document.createElement('style');
    styleTag.textContent = `
        .delete-post-btn, .block-user-btn {
        ...
        .delete-post-btn:hover, .block-user-btn:hover {
            color: #e74c3c;
    ...
    function createAdminUI() {
        ...
        document.getElementById('adminLoginBtn').addEventListener('click', () => {
            if (document.getElementById('adminPasswordInput').value === ADMIN_PASSWORD) {
                ...
                // Добавляем кнопки на существующие посты
                addAdminButtons();
                renderBannedList();
            } else {
        ...
        document.querySelectorAll('.post-card').forEach(card => {
            ...
            if (!card.querySelector('.block-user-btn')) {
                ...
                blockBtn.addEventListener('click', async () => {
                    ...
                    if (confirm(`Заблокировать пользователя ${nick}? Все его посты исчезнут.`)) {
                        await supabase.from('banned_users').upsert({ user_id: userId, nickname: nick });
                        bannedUserIds.add(userId);
                        // Удаляем все посты этого пользователя из DOM
    ...
    async function renderBannedList() {
        ...
        data.forEach(entry => {
            ...
            div.style.cssText = 'display:flex;justify-content:space-between;align-items:center;padding:4px 0;color:white;';
            div.innerHTML = `<span>${escapeHtml(entry.nickname || entry.user_id)}</span>`;
            const unblockBtn = document.createElement('button');
            unblockBtn.textContent = 'Разблокировать';
            unblockBtn.style.cssText = 'background:#2ecc71;border:none;color:white;padding:2px 8px;border-radius:4px;cursor:pointer;font-size:0.75rem;';
    ...
    async function loadPosts() {
        ...
        // Если админ, добавляем кнопки администрирования
    ...
    async function publishPost() {
        ...
        try {
            ...
            await supabase.from('posts').insert({
                ...
                content: postTextarea.value.trim(), likes: 0,
        ...
        realtimeSubscription = supabase.channel('posts-channel')
            .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'posts' }, payload => {
                ...
                if (card) {
                    ...
                    feedEmpty.classList.add('hidden');
                    if (isAdmin) addAdminButtons(); // чтобы новые посты тоже получили кнопки
                }
            })
            .subscribe();
    }
