(function() {
    ...
    async function loadPosts() {
        ...
        if (error) { console.error(error); feedLoading.classList.add('hidden'); return; }
    ...
    async function loadMyPosts() {
        ...
        if (error) { console.error(error); myPostsLoading.classList.add('hidden'); return; }
    ...
    async function loadFollowingPosts() {
        ...
        if (error) { console.error(error); followingLoading.classList.add('hidden'); return; }
    ...
    async function loadVideoPosts() {
        ...
        const { data, error } = await supabase.from('posts').select('*').not('video_url', 'is', null).order('created_at', { ascending: false });
        if (error) { console.error(error); videoLoading.classList.add('hidden'); return; }
    ...
    async function publish() {
        if (isPublishing || !currentUser || !profile) return;
        ...
        try {
            ...
            if (selectedVideo) {
                ...
                const { error } = await supabase.storage.from('post-images').upload(path, selectedVideo);
                if (error) throw error;
            ...
            composerErrorText.textContent = 'Ошибка отправки';
        } finally {
            isPublishing = false;
            updatePublishBtn();
        }
    ...
    removeVideoPreview.addEventListener('click', () => {
        selectedVideo = null;
        videoPreview.classList.add('hidden');
        updatePublishBtn();
    });
    ...
    function setupAdmin() {
        ...
        document.getElementById('adminLogin').addEventListener('click', async () => {
            if (document.getElementById('adminPw').value === 'nobuadmin2024') {
                isAdmin = true;
    ...
    function addAdminButtons() {
        document.querySelectorAll('.post-card').forEach(card => {
            ...
            d.className = 'admin-delete-btn';
            d.innerHTML = '<i class="fa-solid fa-trash"></i>';
            d.style.cssText = 'background:none;border:none;color:#666;cursor:pointer;margin-left:auto;';
            ...
            b.className = 'admin-block-btn';
            b.innerHTML = '<i class="fa-solid fa-ban"></i>';
            b.style.cssText = 'background:none;border:none;color:#666;cursor:pointer;';
            b.addEventListener('click', async () => {
                if (confirm(`Заблокировать ${card.querySelector('.post-card-nickname').textContent}?`)) {
                    await supabase.from('banned_users').upsert({
                        ...
                        nickname: card.querySelector('.post-card-nickname').textContent
                    });
    ...
    function initApp() {
        ...
        postTextarea.addEventListener('input', () => {
            charCounter.textContent = postTextarea.value.length + ' / 500';
            updatePublishBtn();
        });
